...
import { Settings, Save, CreditCard, Building, Plus, Trash2, Loader2, Image as ImageIcon, Percent, CheckCircle, Info, MapPin } from "lucide-react";
...
export default function AjustesPage() {
  ...
  const loadMunicipios = async () => {
    ...
    } catch (e) { console.error("Error loading geo", e); }
  };
    ...
    if (cp.length === 5) {
      ...
      if (resp) {
        ...
        if (todosLosMunicipios.length > 0) {
           const filtrados = todosLosMunicipios.filter(m => parseInt(m.parent_code, 10) === parseInt(resp.id, 10));
           if (filtrados.length > 0 && !poblacion) {
              setPoblacion(filtrados[0].label);
           }
        }
      }
    }
  ...
  const handleAddIva = async () => {
    ...
    setNuevoIvaNombre(""); setNuevoIvaValor(0); fetchAjustes();
  };
  ...
  const handleAddIrpf = async () => {
    ...
    setNuevoIrpfNombre(""); setNuevoIrpfValor(0); fetchAjustes();
  };

  const handleDelete = async (table: string, id: string) => {
    await supabase.from(table).delete().eq("id", id); fetchAjustes();
  };
  ...
  const testGeminiKey = async () => {
    ...
    try {
      ...
      setGeminiStatus(res.ok ? { msg: "✓ OK", type: "green" } : { msg: "✕ Error", type: "red" });
    } catch { setGeminiStatus({ msg: "✕ Error", type: "red" }); }
  };
    ...
    <div className="flex bg-[var(--background)] min-h-screen text-left">
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-20">
          ...
          <div className="bg-white rounded-2xl border border-[var(--border)] shadow-sm p-8 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={nombre} onChange={e => setNombre(e.target.value)} className="w-full p-2.5 rounded-lg border font-bold" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">NIF / CIF</label>
                  <input type="text" value={nif} onChange={e => setNif(e.target.value)} className="w-full p-2.5 rounded-lg border uppercase" />
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">IBAN</label>
                  <input type="text" value={cuentaBancaria} onChange={e => setCuentaBancaria(e.target.value)} className="w-full p-2.5 rounded-lg border font-mono text-sm" />
                </div>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">Dirección Física</label>
                <input type="text" value={direccion} onChange={e => setDireccion(e.target.value)} className="w-full p-2.5 rounded-lg border text-sm" />
              </div>

              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">C.P.</label>
                  <input type="text" maxLength={5} value={cp} onChange={e => setCp(e.target.value)} className="w-full p-2.5 rounded-lg border font-mono text-center" />
                </div>
                <div className="col-span-1">
                  <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">Municipio</label>
                  <input type="text" value={poblacion} onChange={e => setPoblacion(e.target.value)} className="w-full p-2.5 rounded-lg border text-sm" />
                </div>
                <div className="col-span-1">
                  <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">Provincia</label>
                  <input type="text" value={provincia} onChange={e => setProvincia(e.target.value)} className="w-full p-2.5 rounded-lg border text-sm" />
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-blue-50/50 border border-blue-100">
              ...
              {tieneRetencion && (
                <div className="mt-2 text-left">
                  <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">% Retención por defecto</label>
                  <input type="number" value={irpfDefault} onChange={e => setIrpfDefault(parseFloat(e.target.value))} className="w-24 p-2 rounded-lg border border-blue-200" />
                </div>
              )}
            </div>

            <button onClick={handleSavePerfil} disabled={isSaving} className="w-full flex items-center justify-center gap-2 py-3 bg-[var(--accent)] text-white font-bold rounded-xl shadow-md transition-all active:scale-[0.98]">
              {isSaving ? <Loader2 className="animate-spin" /> : <Save size={18} />} Guardar Perfil
            </button>
          </div>
          ...
          <div className="space-y-8">
             <div className="bg-white rounded-2xl border p-8 shadow-sm">
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  <input type="number" value={nuevoIvaValor} onChange={e => setNuevoIvaValor(parseFloat(e.target.value))} className="w-16 p-2 border rounded-lg text-center font-bold" />
                  <button onClick={handleAddIva} className="p-2 bg-green-50 text-green-700 rounded-lg"><Plus size={20}/></button>
                </div>
                <div className="space-y-2 max-h-40 overflow-y-auto">{tiposIva.map(v => (
                  <div key={v.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg group">
                    <span className="text-sm font-bold text-gray-700">{v.nombre} ({v.valor}%)</span>
             ...
             <div className="bg-white rounded-2xl border p-8 shadow-sm">
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  <input type="number" value={nuevoIrpfValor} onChange={e => setNuevoIrpfValor(parseFloat(e.target.value))} className="w-16 p-2 border rounded-lg text-center font-bold" />
                  <button onClick={handleAddIrpf} className="p-2 bg-orange-50 text-orange-700 rounded-lg"><Plus size={20}/></button>
                </div>
                <div className="space-y-2 max-h-40 overflow-y-auto">{tiposIrpf.map(v => (
                  <div key={v.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg group">
                    <span className="text-sm font-bold text-gray-700">{v.nombre} ({v.valor}%)</span>
          ...
          <div className="bg-white rounded-2xl border p-8 shadow-sm">
            <h2 className="text-xl font-bold font-head mb-6 flex items-center gap-2"><CreditCard className="text-blue-600" /> Métodos de Pago</h2>
            <div className="flex gap-2 mb-4">
              <input type="text" value={nuevaForma} onChange={e => setNuevaForma(e.target.value)} placeholder="Ej: Transferencia 30 días" className="flex-1 p-2 border rounded-lg text-sm" />
            ...
            <div className="space-y-2">{formasCobro.map(f => (
              <div key={f.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg group">
                <span className="text-sm font-bold text-gray-700">{f.nombre}</span>
          ...
          <div className="bg-white rounded-2xl border p-8 shadow-sm">
            <h2 className="text-xl font-bold font-head mb-6 flex items-center gap-2">✨ Inteligencia Artificial</h2>
            <div className="space-y-4">
                <label className="block text-[11px] font-bold text-gray-400 uppercase">Google Gemini API KEY</label>
                <div className="flex gap-2">
                  <input type="password" value={geminiKey} onChange={e => setGeminiKey(e.target.value)} placeholder="AIzaSy..." className="flex-1 p-3 border rounded-xl font-mono text-sm" />
                  <button onClick={testGeminiKey} className="px-4 bg-gray-100 hover:bg-gray-200 rounded-xl font-bold">Probar</button>
                </div>
                {geminiStatus.msg && <p className={`text-[10px] font-bold uppercase ${geminiStatus.type === 'green' ? 'text-green-600' : 'text-red-500'}`}>{geminiStatus.msg}</p>}
            </div>
          </div>
