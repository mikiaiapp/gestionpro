           ...
           <div className="flex-1">
              {loading ? (
                 ...
                 <div className="flex flex-col items-center justify-center p-32 text-center">
                    ...
                    <p className="text-gray-400 font-bold">No se encontraron archivos ni carpetas en esta ubicación.</p>
                    <button onClick={() => setCurrentPath("")} className="mt-4 text-[var(--accent)] text-xs font-bold hover:underline">Volver a la Raíz</button>
                 </div>
              ) : (
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6">
                    {filteredFiles.map(file => {
                       const isFolder = !file.id; // En Supabase list, los folders no tienen ID
                       return (
                          <div 
                            key={file.name} 
                            onClick={() => isFolder && setCurrentPath(currentPath ? `${currentPath}/${file.name}` : file.name)}
                            className={`p-4 rounded-2xl border transition-all group flex flex-col justify-between min-h-[140px] relative overflow-hidden bg-white ${isFolder ? 'cursor-pointer border-blue-100 hover:border-blue-400 hover:bg-blue-50/30' : 'hover:border-[var(--accent)] hover:bg-[var(--background)]'}`}
                          >
                             ...
                             <div className="relative z-10">
                                <div className={`mb-2 ${isFolder ? 'text-blue-500' : 'text-gray-400'}`}>
                                   {isFolder ? <FolderOpen size={24} /> : <FileText size={24} />}
                                </div>
                                <div className="font-bold text-[13px] text-gray-700 break-words leading-tight pr-4">
                                   {file.name}
                                </div>
                             </div>
                             ...
                             <div className="relative z-10 flex justify-between items-end mt-4">
                                ...
                                {isFolder && <ChevronRight size={16} className="text-blue-300 group-hover:translate-x-1 transition-transform" />}
                             </div>
                          </div>
                       );
                    })}
