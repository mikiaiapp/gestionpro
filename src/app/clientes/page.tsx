...
export default function ClientesPage() {
  ...
  const handleAddCliente = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("clientes").insert([{ nombre, nif, email, direccion, codigo_postal: cp, poblacion, provincia }]);
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md animate-in fade-in zoom-in duration-200">
              <h2 className="text-xl font-bold mb-6">➕ Añadir Cliente</h2>
              <form onSubmit={handleAddCliente} className="space-y-4">
                <input type="text" placeholder="Razón Social" value={nombre} onChange={(e) => setNombre(e.target.value)} className="w-full p-2.5 rounded-lg border border-[var(--border)]" required />
        ...
        <div className="glass-card bg-white shadow-sm border-[var(--border)] overflow-hidden">
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-20 text-center text-[var(--muted)]">No hay clientes registrados</div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#fcfaf7] border-b border-[var(--border)]">
                    <th className="px-6 py-4 text-[11px] font-bold text-[var(--muted)] uppercase">Cliente</th>
